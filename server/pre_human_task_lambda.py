...
def lambda_handler(event, context):
    ...
    semantic_segment_job_ref = event['dataObject']['semantic-segment-job-ref'] if "semantic-segment-job-ref" in event['dataObject'] else None
    ...
    if semantic_segment_job_ref is not None:
        print(semantic_segment_job_ref)
        output['taskInput']['semantic-segment-job-ref'] = getSegmentedPngAsText(semantic_segment_job_ref)
...
def getSegmentedPngAsText(s3uri):
    ...
    try:
        ...
        text = base64.b64encode(obj.get()['Body'].read()).decode('utf8')
        return text;
    except:
